...
def generate_html_dashboard(data, global_total_minutes, output_file="index.html"):
    ...
    if dates:
        ...
        while cur_y < end_y or (cur_y == end_y and cur_m <= end_m):
            ...
            m_name = f"{month_names_de[cur_m]} {cur_y}"
            m_mins = sum(day["total_minutes"] for dt_str, day in data.items() if dt_str.startswith(m_key))
            ...
            for _ in range(first_weekday):
                grid_cells += '<div class="cal-cell empty"></div>'
            for d in range(1, num_days + 1):
                match_str = f"{cur_y}-{cur_m:02d}-{d:02d}"
                if match_str in data:
                    grid_cells += f"""<div class="cal-cell has-work" onclick="forceDayView('{match_str}')">{d}</div>"""
                else:
    ...
    <head>
        ...
        <style>
            ...
            * {{ box-sizing: border-box; -webkit-tap-highlight-color: transparent; }}
            ...
            .entries-list, .week-list, .month-list {{ flex: 1; overflow-y: auto; -webkit-overflow-scrolling: touch; padding-bottom: 30px; }}

            /* MONATS-SEKTIONEN */
            .month-section {{ margin-bottom: 24px; }}
            .month-header-row {{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; padding: 0 4px; }}
            .month-header-row .calendar-title {{ margin-bottom: 0; padding-left: 0; }}
            .month-hours-badge {{ 
                font-size: 12.5px; font-weight: 700; color: #004999; 
                background: rgba(37, 117, 252, 0.15); border: 1.5px solid rgba(37, 117, 252, 0.4); 
